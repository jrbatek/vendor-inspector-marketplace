...
type FormState = {
  ...
  office_address:string; office_lat:string; office_lng:string;
  base_city:string; base_state:string; base_country:string;
  travel_distance:string; distance_unit:"miles"|"kilometers";
  mileage_rate:string; kilometer_rate:string;
...
const blank: FormState = {
  ...
  base_city:"", base_state:"", base_country:"USA",
  travel_distance:"", distance_unit:"miles",
  mileage_rate:"", kilometer_rate:"",
  certifications:"API 510, API 570, CWI",
  methods:"VT, MT, PT, UT",
  industries:"Oil & Gas, Power, Chemicals",
  hourly_rate:"", day_rate:"", bio:"", available:true
};
...
export default function DashboardPage() {
  ...
  async function validateAddress() {
    ...
    if (!Array.isArray(data) || !data[0]) {
      setMessage("Address not found. Try a more complete address.");
      return;
    }
  ...
  return (
    <section className="panel">
      ...
      {message && <p className={message==="Profile saved." || message==="Address validated." ? "success" : "notice"}>{message}</p>}
      ...
      <form onSubmit={save}>
        <div className="formGrid">
          ...
          <div className="full actions">
            ...
            {form.office_lat && form.office_lng && <span className="muted">Validated: {form.office_lat}, {form.office_lng}</span>}
          </div>
          ...
          <label>Driving Distance
            <input type="number" value={form.travel_distance} onChange={(e)=>setField("travel_distance",e.target.value)} placeholder="250" />
          </label>
          ...
          {form.distance_unit === "miles" ? (
            <label>Mileage Rate
              <input type="number" value={form.mileage_rate} onChange={(e)=>setField("mileage_rate",e.target.value)} placeholder="0.67" />
            </label>
          ) : (
            <label>Kilometer Rate
              <input type="number" value={form.kilometer_rate} onChange={(e)=>setField("kilometer_rate",e.target.value)} placeholder="0.50" />
            </label>
          )}
