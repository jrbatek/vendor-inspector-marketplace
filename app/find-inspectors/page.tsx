...
export default function FindInspectorsPage() {
  ...
  return <main className="shell"><ClientWorkspaceSidebar/><section className="workspace">
      <section className="intakeCard"><p className="eyebrow">InspectSource Client Assistant</p><h1>Tell us what inspection support you need</h1><p className="intro">Describe the assignment naturally. InspectSource converts it into structured requirements, verifies profile facts, and ranks anonymous inspectors by fit.</p><textarea rows={8} value={text} onChange={(event) => setText(event.target.value)} aria-label="Inspection assignment"/><div className="actions"><label className="fileButton">Add scope text file<input type="file" accept=".txt,.md,.csv,.json,.pdf,.doc,.docx" onChange={(event) => void loadTextFile(event.target.files?.[0])}/></label><button type="button" onClick={() => void search()} disabled={searching}>{searching ? "Analyzing requirement..." : "Find Qualified Inspectors"}</button></div>{message && <p className="notice">{message}</p>}</section>
