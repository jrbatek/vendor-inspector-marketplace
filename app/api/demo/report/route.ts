...
export async function GET(request:NextRequest){
  ...
  const fields=[
    ["Project",project],["Supplier",supplier],["Location",location],["Commodity",commodity],
    ["Inspection activity",inspection],["Inspector",inspector]
  ];
  let y=638;
  fields.forEach(([label,value],i)=>{
    const col=i%2,row=Math.floor(i/2),x=44+col*262,yy=y-row*58;
    page.drawRectangle({x,y:yy-34,width:246,height:46,color:light,borderColor:rgb(.86,.89,.93),borderWidth:1});
    page.drawText(label.toUpperCase(),{x:x+10,y:yy-6,size:7,font:bold,color:gray});
    page.drawText(value,{x:x+10,y:yy-23,size:10,font:bold,color:navy,maxWidth:225});
  });

  const section=(title:string,body:string,top:number)=>{
    page.drawText(title,{x:44,y:top,size:12,font:bold,color:navy});
    page.drawText(body,{x:44,y:top-22,size:9.5,font:regular,color:gray,maxWidth:520,lineHeight:14});
  };
  section("Scope performed","Scheduled inspection activities were performed against the client inspection and test plan. Documentation, identification, workmanship and the planned hold/witness points were reviewed for the stage inspected.",448);
